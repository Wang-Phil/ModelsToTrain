...
# 混合模型训练脚本 - 仅使用分类损失（CE Loss）
# 使用 PMC-CLIP 的 ResNet50 图像编码器 + BiomedCLIP 的文本编码器
# 只训练图像编码器，仅使用分类损失（不使用对比损失）
...
# 输出目录
OUTPUT_DIR="output/hybrid_pmcclip_biomedclip_ce_loss_only"
...
# 损失函数权重 - 仅使用分类损失
CLASSIFICATION_LOSS_WEIGHT=1.0    # 分类损失权重（设为1.0，仅使用分类损失）
CONTRASTIVE_LOSS_WEIGHT=0.0       # 对比损失权重（设为0.0，不使用对比损失）
...
print_info ""
print_info "损失函数:"
print_info "  - 分类损失权重: $CLASSIFICATION_LOSS_WEIGHT (仅使用分类损失)"
print_info "  - 对比损失权重: $CONTRASTIVE_LOSS_WEIGHT (不使用对比损失)"
print_info ""
print_info "交叉验证:"
