...
# 知识蒸馏训练脚本
# 使用 BiomedCLIP 作为教师模型，蒸馏到 StarNet（学生模型）
# 文本编码器是冻结的，共享使用 BiomedCLIP 的文本编码器
...
# 输出目录
OUTPUT_DIR="output/distill_biomedclip_to_resnet50_pmcclip_pretrained"
...
# 学生模型图像编码器
# 格式：model_name:pretrained，例如：
#   - resnet50:false 表示不使用预训练权重
#   - resnet50:pmcclip 表示使用PMC-CLIP的预训练权重
...
DISTILL_USE_FOCAL_LOSS=false      # 是否使用Focal Loss
...
print_info "学生模型图像编码器: $STUDENT_IMAGE_ENCODER (使用PMC-CLIP预训练权重)"
print_info "学生模型文本编码器: BiomedCLIP (冻结，与教师共享)"
...
print_info "分类损失权重: $DISTILL_CLASS_LOSS_WEIGHT (Focal Loss: $DISTILL_USE_FOCAL_LOSS, alpha=$DISTILL_FOCAL_ALPHA, gamma=$DISTILL_FOCAL_GAMMA)"
