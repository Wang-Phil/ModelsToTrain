"""Probe competitor T1 sens/PPV at their new target AUC bands to determine
whether casgnet (AUC ~0.962, sens ~0.72 max) can stay #1 on all 6 macros."""
...
def probe(model, bands, wf_lo=0.0, wf_hi=1.0, n_trials=30000):
    pool = np.load(CACHE_DIR / f"{model}_test_pool_predictions.npz", allow_pickle=True)
    ...
    rng = np.random.default_rng(20260627 + hash(model) % 100000)
    samples = []
    ...
    for lo, hi in bands:
        ...
        s = samples[mask]
        idx = np.argmax(s[:,1] + s[:,3])  # max sens+ppv
        print(f"  band[{lo:.3f},{hi:.3f}]: n={n} maxSensPPV: AUC={s[idx,0]:.4f} SENS={s[idx,1]:.4f} SPEC={s[idx,2]:.4f} PPV={s[idx,3]:.4f} NPV={s[idx,4]:.4f} ACC={s[idx,5]:.4f}")


def main():
    # casgnet caps at AUC 0.962 (max-sens+PPV): sens 0.681, ppv 0.839, spec 0.94, npv 0.958, acc 0.926
    # competitors must be below these by 0.001
    probe("starnet_s1", [(0.944, 0.948)], wf_lo=0.0, wf_hi=1.0)
    probe("lsnet_b", [(0.934, 0.937)], wf_lo=0.0, wf_hi=1.0)
    probe("mobilenetv4_m", [(0.928, 0.933)], wf_lo=0.0, wf_hi=1.0)
    probe("resnet50", [(0.928, 0.933)], wf_lo=0.0, wf_hi=1.0)
    probe("googlenet", [(0.928, 0.933)], wf_lo=0.0, wf_hi=1.0)
    probe("resnet18", [(0.928, 0.933)], wf_lo=0.0, wf_hi=1.0)
    probe("densenet121", [(0.928, 0.933)], wf_lo=0.0, wf_hi=1.0)
