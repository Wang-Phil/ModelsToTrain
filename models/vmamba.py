...
WITH_TRITON = False  # 默认禁用 Triton，使用 PyTorch 实现以避免版本兼容性问题
# WITH_TRITON = True
try:
    import triton
    import triton.language as tl
    # 即使导入成功，也默认禁用 Triton（避免版本兼容性问题）
    # WITH_TRITON = True  # 如果需要使用 Triton，可以取消注释
except:
    WITH_TRITON = False
...
# 只在 WITH_TRITON=True 时定义 Triton 函数，避免版本兼容性问题
if WITH_TRITON:
    @triton.jit
    ...
    # 当 WITH_TRITON=False 时，定义一个占位符函数（不会被调用）
    def triton_cross_scan_flex(*args, **kwargs):
        raise RuntimeError("Triton is not available. This function should not be called when WITH_TRITON=False.")
...
class CrossScanTritonF(torch.autograd.Function):
    ...
    def forward(ctx, x: torch.Tensor, in_channel_first=True, out_channel_first=True, one_by_one=False, scans=0):
        ...
        BC, BH, BW = 1, 32, 32
        # 使用 PyTorch 的等价函数替代 triton.cdiv
        if WITH_TRITON:
            NH, NW, NC = triton.cdiv(H, BH), triton.cdiv(W, BW), triton.cdiv(C, BC)
        else:
            NH, NW, NC = (H + BH - 1) // BH, (W + BW - 1) // BW, (C + BC - 1) // BC
...
def selective_scan_torch(
    ...
    **kwargs
    ...
    # 优化：避免创建大的 repeat 张量 (B, KCdim, N, L)，直接在循环中处理
    # 原始代码会将 B 和 C 从 (B, K, N, L) repeat 成 (B, KCdim, N, L)
    # 我们改为在循环中按 K 分支处理，每个分支的值被复制 Cdim 次
    ...
    for i in range(L):
        # 获取当前时间步的数据
        u_i = u[:, :, i]  # (B, KCdim)
        delta_i = delta[:, :, i]  # (B, KCdim)
        B_i = B[:, :, :, i]  # (B, K, N) - 原始形状，未 repeat
        C_i = C[:, :, :, i]  # (B, K, N)
        
        # 按 K 个分支分别处理，每个分支处理 Cdim 个通道
        # 收集所有分支的更新，避免原地操作
        x_parts = []
        y_i_parts = []
        for k in range(K):
            ...
            k_start = k * Cdim
            k_end = (k + 1) * Cdim
            u_k = u_i[:, k_start:k_end]  # (B, Cdim)
            delta_k = delta_i[:, k_start:k_end]  # (B, Cdim)
            A_k = A[k_start:k_end, :]  # (Cdim, N)
            x_k = x[:, k_start:k_end, :]  # (B, Cdim, N)
            
            # B_k 和 C_k 需要广播到 (B, Cdim, N)
            # 原始 B_i[:, k, :] 是 (B, N)，需要扩展为 (B, Cdim, N)
            B_k_expanded = B_i[:, k, :].unsqueeze(1).expand(-1, Cdim, -1)  # (B, Cdim, N)
            C_k_expanded = C_i[:, k, :].unsqueeze(1).expand(-1, Cdim, -1)  # (B, Cdim, N)
            
            # 计算 deltaA: exp(delta * A)
            deltaA_k = torch.exp(delta_k.unsqueeze(2) * A_k.unsqueeze(0))  # (B, Cdim, N)
            
            # 计算 deltaB_u: delta * B * u
            # delta_k: (B, Cdim, 1), B_k_expanded: (B, Cdim, N), u_k: (B, Cdim, 1)
            deltaB_u_k = delta_k.unsqueeze(2) * B_k_expanded * u_k.unsqueeze(2)  # (B, Cdim, N)
            
            # 更新状态（非原地操作）
            x_k_new = deltaA_k * x_k + deltaB_u_k
            x_parts.append(x_k_new)
            
            # 计算输出: (x * C).sum(dim=2)
            y_k = (x_k_new * C_k_expanded).sum(dim=2)  # (B, Cdim)
            y_i_parts.append(y_k)
        
        # 合并所有分支的更新（非原地操作）
        x = torch.cat(x_parts, dim=1)  # (B, KCdim, N)
        
        # 合并所有分支的输出
        y_i = torch.cat(y_i_parts, dim=1)  # (B, KCdim)
        ys.append(y_i)
...
class VSSM(nn.Module):
    ...
    def flops(self, shape=(3, 224, 224), verbose=True):
        ...
        if parameter_count is not None and flop_count is not None:
            params = parameter_count(model)[""]
            Gflops, unsupported = flop_count(model=model, inputs=(input,), supported_ops=supported_ops)
            del model, input
            return sum(Gflops.values()) * 1e9
        else:
            # fvcore not available, return 0
            del model, input
            return 0.0
