...
try:
    ...
    # 检查 custom_bwd 是否支持 cast_inputs 参数
    custom_bwd_sig = inspect.signature(custom_bwd)
    has_cast_inputs = 'cast_inputs' in custom_bwd_sig.parameters
    
    if has_cast_inputs:
        # PyTorch < 2.0 版本，支持 cast_inputs
        USE_DEVICE_TYPE = False
    else:
        # PyTorch 2.0+ 版本，custom_bwd 不支持 cast_inputs，只接受 bwd 函数
        USE_DEVICE_TYPE = False
        # 创建包装函数以兼容旧代码
        _original_custom_fwd = custom_fwd
        _original_custom_bwd = custom_bwd
        
        def custom_fwd_wrapper(fwd=None, cast_inputs=None, device_type=None):
            if fwd is not None:
                return _original_custom_fwd(fwd, cast_inputs=cast_inputs)
            else:
                def decorator(func):
                    return _original_custom_fwd(func, cast_inputs=cast_inputs)
                return decorator
        
        def custom_bwd_wrapper(bwd=None, cast_inputs=None, device_type=None):
            # PyTorch 2.0+ 的 custom_bwd 只接受 bwd 参数，忽略其他参数
            if bwd is not None:
                return _original_custom_bwd(bwd)
            else:
                def decorator(func):
                    return _original_custom_bwd(func)
                return decorator
        
        custom_fwd = custom_fwd_wrapper
        custom_bwd = custom_bwd_wrapper
except ImportError:
    # 如果都导入失败，使用装饰器占位符
    USE_DEVICE_TYPE = False
    def custom_fwd(cast_inputs=None, device_type=None):
        def decorator(func):
            return func
        return decorator
    def custom_bwd(cast_inputs=None, device_type=None, bwd=None):
        def decorator(func):
            return func
        return decorator
import math
...
class SkaFn(Function):
    @staticmethod
    @(custom_fwd(device_type='cuda') if USE_DEVICE_TYPE else custom_fwd(cast_inputs=None))
    ...
    @staticmethod
    @(custom_bwd(device_type='cuda') if USE_DEVICE_TYPE else custom_bwd(cast_inputs=None))
