...
# 尝试从系统路径导入 CLIP 库
clip = None
PUBMEDCLIP_CLIP_AVAILABLE = False
        ...
        if hasattr(_clip, 'load') and hasattr(_clip, 'tokenize'):
            ...
            if clip_file:
                if 'ModelsTotrain/clip' not in clip_file:
                    # 这是系统 CLIP 库
                    clip = _clip
                    PUBMEDCLIP_CLIP_AVAILABLE = True
                else:
                    # 这是本地空模块
                    PUBMEDCLIP_CLIP_AVAILABLE = False
                    clip = None
...
if not PUBMEDCLIP_CLIP_AVAILABLE:
    # 创建一个占位符模块，避免后续导入错误
...
def load_clip_to_cpu(cfg):
    ...
    # 首先检查 CLIP 库是否可用
    if not PUBMEDCLIP_CLIP_AVAILABLE or not hasattr(clip, 'load'):
        raise ImportError(
            f"无法加载 PubMedCLIP 模型：系统未安装标准 CLIP 库。\n"
            "请先安装 CLIP 库：\n"
            "  pip install git+https://github.com/openai/CLIP.git\n"
            "\n"
...
class CustomCLIP(nn.Module):
    """
    原始 PubMedCLIP 模型（仅使用对比损失）
    不使用 CoOp prompt learning，只使用原始的 CLIP 对比学习损失
    """
    def __init__(self, cfg, classnames, clip_model):
        super().__init__()
        # ========== 原始 PubMedCLIP 模型（不使用 CoOp）==========
    ...
    def forward(self, image, label=None):
        ...
        if self.training and label is not None:
            ...
            for text in batch_texts:
                ...
                # 使用 PubMedCLIP 的 encode_text 方法
...
class CoOp_PubMedCLIP(TrainerX):
    ...
    def build_model(self):
        ...
        print("Building original PubMedCLIP (contrastive loss only, no CoOp)")
        self.model = CustomCLIP(cfg, classnames, clip_model)

        # ========== 原始 PubMedCLIP：只训练图像编码器 ==========
        ...
        # ========== 原始 PubMedCLIP：优化器包含所有可训练参数（主要是图像编码器）==========
