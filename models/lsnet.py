...
def _create_lsnet(variant, pretrained=False, **kwargs):
    # 直接创建模型，不使用 build_model_with_cfg 以避免 hf_hub 兼容性问题
    # 如果需要预训练权重，可以通过其他方式加载
    if pretrained:
        print(f"警告: {variant} 的预训练权重加载功能暂未实现，将使用随机初始化")
    
    # 从 kwargs 中提取参数，如果没有则使用默认值
    ...
    # 根据 variant 设置模型参数
    if 'lsnet_t' in variant:
        embed_dim = [64, 128, 256, 384]
        depth = [0, 2, 8, 10]
        num_heads = [3, 3, 3, 4]
    elif 'lsnet_s' in variant:
        embed_dim = [96, 192, 320, 448]
        depth = [1, 2, 8, 10]
        num_heads = [3, 3, 3, 4]
    elif 'lsnet_b' in variant:
