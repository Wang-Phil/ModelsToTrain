...
# ResNet50 (PMC-CLIP 预训练) + BiomedCLIP 文本编码器训练脚本
# 使用 ResNet50（PMC-CLIP 预训练权重）作为图像编码器，BiomedCLIP 的文本编码器进行训练
# 损失函数：仅使用 CLIP 对比损失（不使用分类损失，不使用知识蒸馏）
...
# 输出目录
OUTPUT_DIR="output/resnet50_pmcclip_biomedclip"
...
# ResNet50 模型（使用 PMC-CLIP 预训练权重）
# 使用 resnet50:pmcclip 表示使用 PMC-CLIP 的预训练权重
IMAGE_ENCODER="resnet50:pmcclip"
...
CHECKPOINT_FILE="$CHECKPOINT_DIR/image_encoder(resnet50).pth"
if [ ! -f "$CHECKPOINT_FILE" ]; then
    print_warning "PMC-CLIP 权重文件不存在: $CHECKPOINT_FILE"
    print_info "请从以下 URL 下载:"
    print_info "https://huggingface.co/datasets/axiong/pmc_oa/resolve/main/image_encoder(resnet50).pth"
    print_info "保存到: $CHECKPOINT_DIR"
    print_info ""
    print_info "或者运行以下命令自动下载（如果 models/coop_pmcclip.py 中的下载功能可用）:"
    print_info "python -c \"from models.coop_pmcclip import download_file; download_file('https://huggingface.co/datasets/axiong/pmc_oa/resolve/main/image_encoder(resnet50).pth', '$CHECKPOINT_FILE')\""
    print_info ""
...
print_info "=========================================="
print_info "ResNet50 (PMC-CLIP 预训练) + BiomedCLIP 训练配置"
print_info "=========================================="
print_info "数据目录: $DATA_DIR"
print_info "输出目录: $OUTPUT_DIR"
print_info "图像编码器: $IMAGE_ENCODER (使用 PMC-CLIP 预训练权重)"
...
print_info "损失函数: CLIP 对比损失（仅对比损失，无分类损失）"
