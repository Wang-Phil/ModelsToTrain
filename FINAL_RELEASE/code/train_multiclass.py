...
# 论文 PoC StarNet-S1~S4 / 超小网络在 models.starnet；SK 消融与 grn_only/sa_only 在 models.starnetsk
...
from models.ab_starnet import (
    ab_starnet_s1,
    ab_starnet_baseline,
    ab_starnet_sa_all, ab_starnet_sa_last3,
    ab_starnet_sa_last2, ab_starnet_sa_last1,
    ab_starnet_attn_spatial, ab_starnet_attn_channel, ab_starnet_attn_cbam,
    ab_starnet_sa_grn,
    ab_starnet_sk39_all, ab_starnet_sk39_last3,
    ab_starnet_sk39_last2, ab_starnet_sk39_last1,
    ab_starnet_sk13_last1, ab_starnet_sk15_last1,
    ab_starnet_sk17_last1, ab_starnet_sk19_last1,
    ab_starnet_sk35_last1, ab_starnet_sk37_last1,
    ab_starnet_sk57_last1, ab_starnet_sk59_last1,
    ab_starnet_sk79_last1,
    ab_starnet_casg_ab_sa, ab_starnet_casg_ab_grn, ab_starnet_casg_ab_sk17,
    ab_starnet_casg_ab_sk17_all_sa, ab_starnet_casg_ab_sk17_all_grn,
    ab_starnet_casg_ab_sk17_last_sa, ab_starnet_casg_ab_sk17_last_grn,
    ab_starnet_casg_ab_sa_grn, ab_starnet_casg_ab_sa_sk17,
    ab_starnet_casg_ab_grn_sk17, ab_starnet_casg_ab_full,
    ab_starnet_casg_ab_sa_grn_last2_last_sk17,
)
...
def get_data_augmentation(augmentation_type='standard', img_size=224, legacy_val_resize=False):
    """
    获取数据增强策略
    ...
    Args:
        ...
        img_size: 图像大小
        legacy_val_resize: 若为 True，验证/测试使用 Resize((img_size, img_size))（与
            old_data_supcon_compare_v2 训练及 bootstrap 表一致）；默认 False 为等比缩放+中心裁剪。
    """
    # Helper: 等比例缩放 + 中心裁剪 + 必要时 padding，保持长宽比不变
    def _aspect_preserve_resize_crop_pad(img: Image.Image, target_size: int) -> Image.Image:
        w, h = img.size
        # 如果任一边为0，直接返回空白目标图（防御性处理）
        if w == 0 or h == 0:
            return Image.new(img.mode, (target_size, target_size))

        scale = float(target_size) / min(w, h)
        ...
        # 如果任一维度小于目标，先 pad 再 center crop；否则直接 center crop
        if new_w >= target_size and new_h >= target_size:
            img = TF.center_crop(img, (target_size, target_size))
        else:
    ...
    if augmentation_type == 'none':
        train_transform = transforms.Compose([
            transforms.Lambda(lambda img: _aspect_preserve_resize_crop_pad(img, img_size)),
            transforms.ToTensor(),
            normalize,
        ])
    ...
    # 验证/测试：默认等比例缩放 + 中心裁剪；legacy 模式与旧实验一致（直接拉伸到正方形）
    if legacy_val_resize:
...
def create_model(model_name, num_classes=9, pretrained=False, **kwargs):
    ...
    AbStarNet (StarNet + Spatial Attention):
        - ab_starnet_baseline (base_dim=24, depths=[2,2,8,3], 纯基线: 无SA、无GRN)
        - ab_starnet_s1      (base_dim=24, depths=[2,2,8,3], 所有Stage均加SA)
    AbStarNet 注意力位置消融实验 (base: S1, depths=[2,2,8,3]):
        - ab_starnet_sa_all   (消融-1: 全部4个Stage加SA: Stage 0-3)
        - ab_starnet_sa_last3 (消融-2: 后3个Stage加SA: Stage 1-3)
        - ab_starnet_sa_last2 (消融-3: 后2个Stage加SA: Stage 2-3)
        - ab_starnet_sa_last1 (消融-4: 仅最后Stage加SA: Stage 3)
    AbStarNet 注意力类别消融实验 (全部Stage加注意力, base: S1):
        - ab_starnet_attn_spatial (类别消融-1: 空间注意力 SpatialAttention)
        - ab_starnet_attn_channel (类别消融-2: 通道注意力 ChannelAttention)
        - ab_starnet_attn_cbam    (类别消融-3: CBAM = Channel + Spatial)
    AbStarNet SA + GRN 联合实验 (全部Block加SA + GRN):
        - ab_starnet_sa_grn (全部Block: SpatialAttention + GRN)
    AbStarNet SKUnit 位置消融实验 (kernel=[3,9], 无attn, 无GRN):
        - ab_starnet_sk39_all   (SK消融-1: 全部4个Stage使用SKBlock)
        - ab_starnet_sk39_last3 (SK消融-2: 后3个Stage使用SKBlock, Stage 1-3)
        - ab_starnet_sk39_last2 (SK消融-3: 后2个Stage使用SKBlock, Stage 2-3)
        - ab_starnet_sk39_last1 (SK消融-4: 仅最后Stage使用SKBlock, Stage 3)
    AbStarNet SKUnit 卷积核大小消融实验 (仅 Stage3, 无attn, 无GRN):
        - ab_starnet_sk13_last1 (kernel=[1,3])
        - ab_starnet_sk15_last1 (kernel=[1,5])
        - ab_starnet_sk17_last1 (kernel=[1,7])
        - ab_starnet_sk19_last1 (kernel=[1,9])
        - ab_starnet_sk35_last1 (kernel=[3,5])
        - ab_starnet_sk37_last1 (kernel=[3,7])
        - ab_starnet_sk39_last1 (kernel=[3,9], 见上方位置消融)
        - ab_starnet_sk57_last1 (kernel=[5,7])
        - ab_starnet_sk59_last1 (kernel=[5,9])
        - ab_starnet_sk79_last1 (kernel=[7,9])
        - ab_starnet_casg_ab_sa (CASG 整体消融1: 仅 SA)
        - ab_starnet_casg_ab_grn (CASG 整体消融2: 仅 GRN)
        - ab_starnet_casg_ab_sk17 (CASG 整体消融3: 仅最后 Stage SK [1,7])
        - ab_starnet_casg_ab_sk17_all_sa (sk17 + Stage3 每个 SKBlock 前加 SA；last_sa 为同名旧称)
        - ab_starnet_casg_ab_sk17_all_grn (sk17 + 每个 SKBlock Star 后 GRN；last_grn 为同名旧称)
        - ab_starnet_casg_ab_sa_grn (CASG 整体消融4: SA+GRN)
        - ab_starnet_casg_ab_sa_sk17 (CASG 整体消融5: SA+SK[1,7] last stage)
        - ab_starnet_casg_ab_grn_sk17 (CASG 整体消融6: GRN+SK[1,7] last stage)
        - ab_starnet_casg_ab_full (CASG 整体消融7: Stage0–2 为 SA+GRN；Stage3 三格均为 SA→SKUnit[1,7]→Star→GRN)
        - ab_starnet_casg_ab_sa_grn_last2_last_sk17 (CASG 整体消融8: Stage0–2 为 SA+GRN；Stage3 前两格 SA+GRN，末格仅 SKUnit[1,7])
    """
    ...
    # CASGNet models
    elif model_name == 'casgnet_s1':
        return casgnet_s1(pretrained=pretrained, num_classes=num_classes, **kwargs)
    elif model_name == 'casgnet_s2':
        return casgnet_s2(pretrained=pretrained, num_classes=num_classes, **kwargs)
    elif model_name == 'casgnet_s1_ab000':
        return casgnet_s1_ab000(pretrained=pretrained, num_classes=num_classes, **kwargs)
    elif model_name == 'casgnet_s1_ab100':
        return casgnet_s1_ab100(pretrained=pretrained, num_classes=num_classes, **kwargs)
    elif model_name == 'casgnet_s1_ab010':
        return casgnet_s1_ab010(pretrained=pretrained, num_classes=num_classes, **kwargs)
    elif model_name == 'casgnet_s1_ab001':
        return casgnet_s1_ab001(pretrained=pretrained, num_classes=num_classes, **kwargs)
    elif model_name == 'casgnet_s1_ab110':
        return casgnet_s1_ab110(pretrained=pretrained, num_classes=num_classes, **kwargs)
    elif model_name == 'casgnet_s1_ab101':
        return casgnet_s1_ab101(pretrained=pretrained, num_classes=num_classes, **kwargs)
    elif model_name == 'casgnet_s1_ab011':
        return casgnet_s1_ab011(pretrained=pretrained, num_classes=num_classes, **kwargs)
    elif model_name == 'casgnet_s1_ab111':
        return casgnet_s1_ab111(pretrained=pretrained, num_classes=num_classes, **kwargs)

    # AbStarNet 纯基线 (无SA, 无GRN)
    elif model_name == 'ab_starnet_baseline':
        return ab_starnet_baseline(pretrained=pretrained, num_classes=num_classes)
    # AbStarNet models (StarNet + Spatial Attention)
    elif model_name == 'ab_starnet_s1':
        return ab_starnet_s1(pretrained=pretrained, num_classes=num_classes)
    # AbStarNet 注意力位置消融实验
    elif model_name == 'ab_starnet_sa_all':
        return ab_starnet_sa_all(pretrained=pretrained, num_classes=num_classes)
    elif model_name == 'ab_starnet_sa_last3':
        return ab_starnet_sa_last3(pretrained=pretrained, num_classes=num_classes)
    elif model_name == 'ab_starnet_sa_last2':
        return ab_starnet_sa_last2(pretrained=pretrained, num_classes=num_classes)
    elif model_name == 'ab_starnet_sa_last1':
        return ab_starnet_sa_last1(pretrained=pretrained, num_classes=num_classes)
    # AbStarNet 注意力类别消融实验
    elif model_name == 'ab_starnet_attn_spatial':
        return ab_starnet_attn_spatial(pretrained=pretrained, num_classes=num_classes)
    elif model_name == 'ab_starnet_attn_channel':
        return ab_starnet_attn_channel(pretrained=pretrained, num_classes=num_classes)
    elif model_name == 'ab_starnet_attn_cbam':
        return ab_starnet_attn_cbam(pretrained=pretrained, num_classes=num_classes)
    # AbStarNet SA + GRN 联合实验
    elif model_name == 'ab_starnet_sa_grn':
        return ab_starnet_sa_grn(pretrained=pretrained, num_classes=num_classes)
    # AbStarNet SKUnit 位置消融实验 (kernel=[3,9], 无attn, 无GRN)
    elif model_name == 'ab_starnet_sk39_all':
        return ab_starnet_sk39_all(pretrained=pretrained, num_classes=num_classes)
    elif model_name == 'ab_starnet_sk39_last3':
        return ab_starnet_sk39_last3(pretrained=pretrained, num_classes=num_classes)
    elif model_name == 'ab_starnet_sk39_last2':
        return ab_starnet_sk39_last2(pretrained=pretrained, num_classes=num_classes)
    elif model_name == 'ab_starnet_sk39_last1':
        return ab_starnet_sk39_last1(pretrained=pretrained, num_classes=num_classes)
    # SKUnit 卷积核大小消融实验 (仅 Stage3, 无attn, 无GRN)
    elif model_name == 'ab_starnet_sk13_last1':
        return ab_starnet_sk13_last1(pretrained=pretrained, num_classes=num_classes)
    elif model_name == 'ab_starnet_sk15_last1':
        return ab_starnet_sk15_last1(pretrained=pretrained, num_classes=num_classes)
    elif model_name == 'ab_starnet_sk17_last1':
        return ab_starnet_sk17_last1(pretrained=pretrained, num_classes=num_classes)
    elif model_name == 'ab_starnet_sk19_last1':
        return ab_starnet_sk19_last1(pretrained=pretrained, num_classes=num_classes)
    elif model_name == 'ab_starnet_sk35_last1':
        return ab_starnet_sk35_last1(pretrained=pretrained, num_classes=num_classes)
    elif model_name == 'ab_starnet_sk37_last1':
        return ab_starnet_sk37_last1(pretrained=pretrained, num_classes=num_classes)
    elif model_name == 'ab_starnet_sk57_last1':
        return ab_starnet_sk57_last1(pretrained=pretrained, num_classes=num_classes)
    elif model_name == 'ab_starnet_sk59_last1':
        return ab_starnet_sk59_last1(pretrained=pretrained, num_classes=num_classes)
    elif model_name == 'ab_starnet_sk79_last1':
        return ab_starnet_sk79_last1(pretrained=pretrained, num_classes=num_classes)

    # CASGNet 整体消融 (StarNet-S1，日志/权重目录见 run_multi_model_cv casgnet_ab_all)
    elif model_name == 'ab_starnet_casg_ab_sa':
        return ab_starnet_casg_ab_sa(pretrained=pretrained, num_classes=num_classes)
    elif model_name == 'ab_starnet_casg_ab_grn':
        return ab_starnet_casg_ab_grn(pretrained=pretrained, num_classes=num_classes)
    elif model_name == 'ab_starnet_casg_ab_sk17':
        return ab_starnet_casg_ab_sk17(pretrained=pretrained, num_classes=num_classes)
    elif model_name == 'ab_starnet_casg_ab_sk17_all_sa':
        return ab_starnet_casg_ab_sk17_all_sa(pretrained=pretrained, num_classes=num_classes)
    elif model_name == 'ab_starnet_casg_ab_sk17_all_grn':
        return ab_starnet_casg_ab_sk17_all_grn(pretrained=pretrained, num_classes=num_classes)
    elif model_name == 'ab_starnet_casg_ab_sk17_last_sa':
        return ab_starnet_casg_ab_sk17_last_sa(pretrained=pretrained, num_classes=num_classes)
    elif model_name == 'ab_starnet_casg_ab_sk17_last_grn':
        return ab_starnet_casg_ab_sk17_last_grn(pretrained=pretrained, num_classes=num_classes)
    elif model_name == 'ab_starnet_casg_ab_sa_grn':
        return ab_starnet_casg_ab_sa_grn(pretrained=pretrained, num_classes=num_classes)
    elif model_name == 'ab_starnet_casg_ab_sa_sk17':
        return ab_starnet_casg_ab_sa_sk17(pretrained=pretrained, num_classes=num_classes)
    elif model_name == 'ab_starnet_casg_ab_grn_sk17':
        return ab_starnet_casg_ab_grn_sk17(pretrained=pretrained, num_classes=num_classes)
    elif model_name == 'ab_starnet_casg_ab_full':
        return ab_starnet_casg_ab_full(pretrained=pretrained, num_classes=num_classes)
    elif model_name == 'ab_starnet_casg_ab_sa_grn_last2_last_sk17':
        return ab_starnet_casg_ab_sa_grn_last2_last_sk17(pretrained=pretrained, num_classes=num_classes)
