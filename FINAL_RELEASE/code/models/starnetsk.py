...
# 官方 StarNet ImageNet 预训练权重 (Rewrite-the-Stars)
...
def _load_pretrained_starnet(model, model_name, num_classes):
    """加载 StarNet 官方 ImageNet 预训练权重（backbone + norm），分类头按 num_classes 随机初始化。"""
    ...
    try:
        ...
        # .pth.tar 常见格式: {'state_dict': ..., 'epoch': ...} 或直接 state_dict
        state_dict = checkpoint.get("state_dict", checkpoint)
        if not isinstance(state_dict, dict):
            return model
        # 去掉 DDP 的 'module.' 前缀
        state_dict = {k.replace("module.", ""): v for k, v in state_dict.items()}
        # 预训练 head 是 1000 类，当前任务可能不同，只加载 backbone 部分（排除 head）
        model_state = model.state_dict()
        loaded = {k: v for k, v in state_dict.items() if k in model_state and model_state[k].shape == v.shape}
        if loaded:
            model_state.update(loaded)
            model.load_state_dict(model_state, strict=False)
            print(f"[StarNet] 已加载 {model_name} 预训练权重: {len(loaded)} 个参数 (分类头按 num_classes={num_classes} 重新初始化)")
        else:
            print(f"[StarNet] 预训练权重与当前模型结构不匹配，跳过加载。")
    except Exception as e:
        print(f"[StarNet] 加载预训练权重失败 ({url}): {e}，使用随机初始化")
    return model
...
def starnet_s3(pretrained=False, **kwargs):
    ...
    if pretrained:
        num_classes = kwargs.get("num_classes", 1000)
        _load_pretrained_starnet(model, "starnet_s3", num_classes)
    return model
...
def starnet_s4(pretrained=False, **kwargs):
    ...
    if pretrained:
        num_classes = kwargs.get("num_classes", 1000)
        _load_pretrained_starnet(model, "starnet_s4", num_classes)
    return model
