...
# CASGNet-S1 消融命名 casgnet_s1_abXYZ：X=SpatialAttention，Y=GRN，Z=末 stage SKSGBlock（SelectiveKernel 门控分支）
...
class ASGBlock(nn.Module):
    def __init__(
        ...
        self.sa = SpatialAttention(kernel_size=sa_kernel_size) if use_sa else nn.Identity()
    ...
    def __init__(
        ...
        for i_layer, depth in enumerate(depths):
            ...
            for i in range(depth):
                is_last_stage = i_layer == len(depths) - 1
                if is_last_stage and use_sk_last:
                    blk = SKSGBlock(
                        self.in_channel,
                        mlp_ratio=mlp_ratio,
                        drop_path=dpr[cur + i],
...
def _casgnet_s1_ablation(use_sa: bool, use_grn: bool, use_sk_last: bool, pretrained: bool = False, **kwargs):
    """S1 档位 2^3 消融：位序 (SA, GRN, SK_last)；与默认 casgnet_s1 对齐 sk_kernel_sizes=(3,9)。"""
    model = CASGNet(
        base_dim=24,
        depths=(2, 2, 8, 3),
        mlp_ratio=4,
...
@register_model
def casgnet_s1_ab000(pretrained=False, **kwargs):
    """消融 SA=0 GRN=0 SK=0（各 stage 均为无 SK 的 ASGBlock，SA/GRN 关闭 → 类 StarNet 主干）"""
    return _casgnet_s1_ablation(False, False, False, pretrained=pretrained, **kwargs)


@register_model
def casgnet_s1_ab100(pretrained=False, **kwargs):
    """仅 SA"""
    return _casgnet_s1_ablation(True, False, False, pretrained=pretrained, **kwargs)


@register_model
def casgnet_s1_ab010(pretrained=False, **kwargs):
    """仅 GRN"""
    return _casgnet_s1_ablation(False, True, False, pretrained=pretrained, **kwargs)


@register_model
def casgnet_s1_ab001(pretrained=False, **kwargs):
    """仅末 stage SK"""
    return _casgnet_s1_ablation(False, False, True, pretrained=pretrained, **kwargs)


@register_model
def casgnet_s1_ab110(pretrained=False, **kwargs):
    """SA + GRN"""
    return _casgnet_s1_ablation(True, True, False, pretrained=pretrained, **kwargs)


@register_model
def casgnet_s1_ab101(pretrained=False, **kwargs):
    """SA + SK"""
    return _casgnet_s1_ablation(True, False, True, pretrained=pretrained, **kwargs)


@register_model
def casgnet_s1_ab011(pretrained=False, **kwargs):
    """GRN + SK"""
    return _casgnet_s1_ablation(False, True, True, pretrained=pretrained, **kwargs)


@register_model
def casgnet_s1_ab111(pretrained=False, **kwargs):
    """完整 CASGNet-S1（等价默认开关：SA+GRN+末 stage SK）"""
    return _casgnet_s1_ablation(True, True, True, pretrained=pretrained, **kwargs)
