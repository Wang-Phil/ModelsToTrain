"""LSNet — vendored from https://github.com/jameslahm/lsnet for timm registration (SupCon / classification)."""
...
class Attention(torch.nn.Module):
    def __init__(self, dim, key_dim, num_heads=8, attn_ratio=4, resolution=14):
        ...
        self.proj = torch.nn.Sequential(torch.nn.ReLU(), Conv2d_BN(self.dh, dim, bn_weight_init=0))
    ...
    def __init__(
        ...
        for i, (ed, kd, dpth, nh, ar) in enumerate(zip(embed_dim, key_dim, depth, num_heads, attn_ratio)):
            ...
            if i != len(depth) - 1:
                ...
                blk.append(
                    Conv2d_BN(embed_dim[i], embed_dim[i], ks=3, stride=2, pad=1, groups=embed_dim[i])
                )
...
def _cfg(url="", hf_hub_id=None, **kwargs):
    """PretrainedCfg-compatible dict for timm >= 1.0."""
    return {
        "url": url or None,
        "hf_hub_id": hf_hub_id,
        "num_classes": 1000,
...
def _create_lsnet(variant, pretrained=False, **kwargs):
    # Variant must match a key in ``default_cfgs`` (used by timm resolve_pretrained_cfg).
...
def lsnet_t(num_classes=1000, distillation=False, pretrained=False, **kwargs):
    return _create_lsnet(
        "lsnet_t" + ("_distill" if distillation else ""),
...
def lsnet_s(num_classes=1000, distillation=False, pretrained=False, **kwargs):
    return _create_lsnet(
        "lsnet_s" + ("_distill" if distillation else ""),
        pretrained=pretrained,
        num_classes=num_classes,
        distillation=distillation,
        img_size=224,
...
def lsnet_b(num_classes=1000, distillation=False, pretrained=False, **kwargs):
    return _create_lsnet(
        "lsnet_b" + ("_distill" if distillation else ""),
        pretrained=pretrained,
        num_classes=num_classes,
        distillation=distillation,
        img_size=224,
