...
# 只训练图像编码器，使用分类损失 + 对比损失 + 蒸馏损失
# 蒸馏：将 BiomedCLIP 图像编码器的知识蒸馏到 PMC-CLIP ResNet50
...
# 输出目录
OUTPUT_DIR="output/hybrid_pmcclip_biomedclip_with_distillation_new_v2_projection"
...
DISTILLATION_LOSS_WEIGHT=0.34      # 蒸馏损失权重（BiomedCLIP -> PMC-CLIP ResNet50）
